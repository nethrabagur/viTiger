...
test('New_Products',async ({page}) => {
    ...
    //! Validating the Newly Created Product
    let p=await page.locator(`//span[@id="dtlview_Product Name"]`).textContent()

    if (p === Product) {
        console.log("A New Product is created & it is ", `${p}`);
    } else {
        console.log("Product is yet to be created");
